...
class ALUS_090_AudioNerve90(NerveBase):
    "\"\"
    SPATIAL AUDIO NERVE 90
    Autonomous neural node designed to emit Doppler audio for Entity 90.
    "\"\"
    NERVE_ID = "ALUS_090"
    DEPARTMENT = "ALUS"
    DIVISION = "audio"
    ...
    def __init__(self):
        super().__init__()
        self.entity_index = 90
        self.audio_thread = threading.Thread(target=self._audio_loop, daemon=True)
        self.audio_thread.start()

    def _audio_loop(self):
        print("[ALUS_090] Spatial Audio Node 90 ACTIVE.")
