...
class CAIN_{id_str}_GlialCellNerve{id}(NerveBase):
    ...
    def _glia_loop(self):
        print("[CAIN_{id_str}] Glial Cell (Microglia) ACTIVE. Immune response online.")
        while True:
...
for i in range(31, 61):
    id_str = f"{i:03d}"
    content = template.replace("{id_str}", id_str).replace("{id}", str(i))
    with open(f"departments/cain/nerves/cain_{id_str}_glial_cell_nerve_{i}.py", "w") as f:
        f.write(content)

print("Generated CAIN 031-060 Microglia Immune System!")
